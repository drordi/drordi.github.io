...
let numOfWordale = findGetParameter('word');
...
function sendWord() {
    ...
    if (win === false) {
        if (wordCount === 6) {
            localStorage.setItem(`answersColors_${numOfWordale}`, []);
            localStorage.setItem(`answersLetters_${numOfWordale}`, []);
            location.reload();
            return;
        }
...
function compareWords() {
    ...
    if (wordCount === 6 && greenIndices.length != 5) {
        ...
        openNotificationLong("בובי אני סומך עליך! נסה שוב", false);
        document.getElementById("send").innerHTML = "איפוס משחק";
    }
}
...
function saveUserData() {
    ...
    localStorage.setItem(`answersColors_${numOfWordale}`, answersColors);
    localStorage.setItem(`answersLetters_${numOfWordale}`, answersLetters);
...
function loadUserData() {
    ...
    if (markAsStarted()) {
        ...
        answersLetters = localStorage.getItem(`answersLetters_${numOfWordale}`);

        if (answersLetters) {
            answersLetters = answersLetters.split(",");
        } else {
            answersLetters = [];
        }
